...
${bodyContent}
  ...
  _handleTable(content) {
    ...
    // ── Emit <colgroup> so column widths are preserved in the HTML output ──
    const hasColWidths = colspec.some(cs => cs && cs[1] && cs[1].t === 'ColWidth');
    if (hasColWidths) {
      // Collect fractions; guard against legacy array-wrapped values
      const fracs = colspec.map(cs => {
      ...
      const total = fracs.reduce((s, f) => s + (f != null ? f : 0), 0);
      if (total > 0) {
        htmlParts.push('<colgroup>');
        for (const f of fracs) {
          const pct = f != null ? ((f / total) * 100).toFixed(2) : (100 / fracs.length).toFixed(2);
          htmlParts.push(`<col style="width:${pct}%"/>`);
        }
        htmlParts.push('</colgroup>');
      }
    }
